  ...
  <section class="cards">
    <div class="card">
      ...
      <div class="kv">
        ...
        <div>BL</div><div><?= h($shipment['bl_number']) ?></div>
        <div>Shipping Code</div><div><?= h($shipment['shipping_code']) ?></div>
        <div>Status</div><div><?= h($shipment['status']) ?></div>
        <div>Origin</div><div><?= h($shipment['origin']) ?></div>
        <div>Destination</div><div><?= h($shipment['destination']) ?></div>
        <div>Pickup</div><div><?= h($shipment['pickup_date']) ?></div>
        <div>Delivery</div><div><?= h($shipment['delivery_date']) ?></div>
        <div>CBM</div><div><?= h($shipment['cbm']) ?></div>
        <div>Cartons</div><div><?= h($shipment['cartons']) ?></div>
        <div>Weight</div><div><?= h($shipment['weight']) ?></div>
        <div>Gross Weight</div><div><?= h($shipment['gross_weight']) ?></div>
        <div>Total Amount</div><div><?= h($shipment['total_amount']) ?></div>
        <div>Created</div><div><?= h($shipment['created_at']) ?></div>
      </div>
      <h3>Description</h3>
